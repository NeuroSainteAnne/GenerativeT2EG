...
def t2egnifti2array(t2egpath, mask, z_pad=None):
    t2egnib = nib.load(t2egpath)
    t2eg = t2egnib.get_fdata()
    
    # pad
    t2eg, padspecs = padvolume(t2eg)
    if z_pad is not None:
        t2eg = zpadding(t2eg, z_pad)
        
    # normalisation
    t2eg = normalize(t2eg, mask)
...
def array2nifti(savearray, padspecs, affine, outpath):
    syntht2eg = savearray[:,:,::-1,0].transpose(2,1,0)
    syntht2eg = syntht2eg - syntht2eg.min()
    syntht2eg = 256*(syntht2eg / syntht2eg.max())
    syntht2eg = reversepad(syntht2eg, padspecs)
    syntht2egnib = nib.Nifti1Image(syntht2eg, affine=affine)
    nib.save(syntht2egnib, outpath)