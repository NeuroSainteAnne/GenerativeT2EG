...
def Generator(n_filters=64, dropout=0.5):
    ...
    u8_mapout = conv_block(u7, n_filters, 3, name="G_Up_3a_Map")
    u8_mapout = Dropout(dropout, name="G_Up_Drop3_Map")(u8_mapout)
    u8_mapout = conv_block(u8_mapout, n_filters, 3, name="G_Up_3b_Map")
    u8_mapout = conv_block(u8_mapout, n_filters, 3, name="G_Up_3c_Map")
    u8_mapout = Conv2D(2, 1, dtype='float32', name="G_Up_3d_Map") (u8_mapout)
    u8_mapout_map = Conv2D(1, 1, dtype='float32', name="G_Up_3e_Map")(u8_mapout[...,0:1])
    u8_mapout_segm = Conv2D(1, 1, dtype='float32', name="G_Up_3e_Segm")(u8_mapout[...,1:2])
    output_segmentation_coarse = sigmoid(u8_mapout_segm)
    output_prediction_coarse = sigmoid(u8_mapout_map)
    ...
    model = Model(inputs=[input_img, input_quality, input_FS], outputs=[output, output_segmentation_coarse, output_prediction_coarse], name="synthT2egGenerator")
    return model
